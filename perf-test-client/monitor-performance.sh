...
# Performance Monitoring Script for BRM API Server
# This script uses pidstat to monitor the Java process running the BRM API server
...
# Function to find the BRM API server process
find_server_pid() {
    # Look for the specific Java process running brm-apiserver
    local pids=$(pgrep -f "java.*brm-apiserver.*jar")
    
    if [ -z "$pids" ]; then
        echo "Error: No BRM API server process found. Make sure the server is running." >&2
    ...
    echo "Found BRM API server process: PID $pid" >&2
    echo "$pid"
}
...
show_usage() {
    ...
    echo ""
    echo "Monitor BRM API server performance using pidstat"
    echo ""
    echo "Options:"
